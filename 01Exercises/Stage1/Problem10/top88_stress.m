...
while change > 0.003
    ...
    relaxed_stress_p = relaxed_stress.^pnorm;
    sum_vmstress_p = sum(relaxed_stress_p(:));
    ...
    dfsdxPhysPart1 = normalized_coefficient * sum_vmstress_p * q * relaxed_stress.^(pnorm-1);


    %fval(2)  = normalized_vmstress_pnorm - sigma;
    ...
    if ft == 3 && beta < 512 && (loopbeta >= 50 || change <= 0.005)
        beta = 2*beta;
        loopbeta = 0;
        change = 1;
