...
function [mu_x_all, mu_y_all, path_distance, xtick_pos, xtick_labels] = generate_band_path_OABCOB()
    % 各段点数配置
    n_segment = 20;  
    n_diag = 28;     
    ...
    x1 = linspace(0, pi, n_segment);
    y1 = zeros(1, n_segment);
    d1 = linspace(0, pi, n_segment);
    
    % A→B (π,0)->(π,π)
    x2 = pi * ones(1, n_segment);
    y2 = linspace(0, pi, n_segment);
    d2 = linspace(pi, 2*pi, n_segment);
    
    % B→C (π,π)->(0,π)
    x3 = linspace(pi, 0, n_segment);
    y3 = pi * ones(1, n_segment);
    d3 = linspace(2*pi, 3*pi, n_segment);
    
    % C→O (0,π)->(0,0)
    x4 = zeros(1, n_segment);
    y4 = linspace(pi, 0, n_segment);
    d4 = linspace(3*pi, 4*pi, n_segment);
    
    % O→B (0,0)->(π,π)
    x5 = linspace(0, pi, n_diag);
    y5 = linspace(0, pi, n_diag);
    diag_length = pi*sqrt(2);
    d5 = linspace(4*pi, 4*pi + diag_length, n_diag);
    ...
    % ================ 计算刻度位置 ================
    % 获取各段起始点的归一化位置
    total_length = 4*pi + diag_length;
    xtick_raw = [0, pi, 2*pi, 3*pi, 4*pi, 4*pi + diag_length];
    xtick_pos = xtick_raw / total_length;  % 归一化
    
    % 对应高对称点标签
    xtick_labels = {'O','A','B','C','O','B'};
