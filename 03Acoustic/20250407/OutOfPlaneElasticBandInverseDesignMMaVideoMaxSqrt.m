...
while change>0.01 || loop<20
    ...
    fprintf('Iter:%4i max:%7.4f apxmax:%7.4f min:%7.4f apxmin:%7.4f gap:%7.4f apxgap:%7.4f Vol:%5.3f chg:%5.3f\n', ...
        loop, max(eigenvalues(:,mode)), apxmax, min(eigenvalues(:,mode+1)), apxmin,  min(eigenvalues(:,mode+1))-max(eigenvalues(:,mode)),apxmax-apxmin, mean(x(:)), change);
end
time = toc;
xfinal = x;
