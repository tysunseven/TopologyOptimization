...
while change>0.01 || loop<20
    ...
    parfor i = 1:length(mu_x_all)
        ...
        eigenvalues(i,:) = sort(sqrt(abs(real(diag(Dcycle)))));
        ...
        dcnmax = dcnmax + sqrt(real(Dcycle(mode,mode)))^(p-1)*real((ceKmax-Dcycle(mode,mode)*ceMmax)/(2*sqrt(real(Dcycle(mode,mode)))*phi'*Ma*phi));
        ...
        dcnplus1min = dcnplus1min + sqrt(real(Dcycle(mode+1,mode+1)))^(q-1)*real((ceKmin-Dcycle(mode+1,mode+1)*ceMmin)/(2*sqrt(real(Dcycle(mode+1,mode+1)))*psi'*Ma*psi));
    end
